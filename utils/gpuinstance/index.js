...
export default class WebGPU {
     ...
     * @param {object} GPURequestAdapterOptions
     * @param {string} GPURequestAdapterOptions.powerPreference powerPreference 为枚举值 low-power high-performance
     * @param {object} GPUObjectDescriptorBase
     * @param {string} GPUObjectDescriptorBase.extensions extensions 只有一个值  用于指定当前绘制的扩展
     * @param {string} GPUObjectDescriptorBase.limits limits 则有一堆选项 规定了获取到的设备的能力上限
     */
    init = async (GPURequestAdapterOptions, GPUObjectDescriptorBase) => {
        /* 获取GPU */
        const { gpu } = window.navigator
        const { powerPreference = 'high-performance' } = GPURequestAdapterOptions || {}
        const { extensions = 'anisotropic-filtering' } = GPUObjectDescriptorBase || {}
    ...
    /**
     * @description 获取 SwapChain 交换链 用于后续向 canvas 输送绘制完毕的图像
     */
     ...
     * @param {*} param1 
     */
    createRenderPass = (name, { width, height, color }) => {
    ...
    createRenderPipeline = (name, { fxCode, vxCode }) => {
        ...
        const vertexBuffers = [{ arrayStride: 4 * 3, attributes: [{ shaderLocation: 0, offset: 0, format: 'float4' }] }]
    ...
    createDeviceBuffer = ($array, $usage, $uint32) => {
        ...
        const DataSource = $uint32 ? new Uint32Array(datasource) : new Float32Array(datasource)
        const { byteLength: size } = DataSource
        
        // 使用 GPU 设备创建了一个GPU缓存用于储存顶点坐标位置
        const GPUBuffer = this.device.createBuffer({ size, usage, mappedAtCreation: true })

        this.device.defaultQueue.writeBuffer(GPUBuffer, 0, DataSource)
        // 使用类型化数组和 ArrayBuffer 的一系列操作，将传入的数据绑定到这个映射缓存的 CPU 部分
        // new DataSource.constructor(GPUBuffer.getMappedRange()).set(DataSource, 0)

        // GPUBuffer.unmap() // 关闭映射操作
        return GPUBuffer
    }
}